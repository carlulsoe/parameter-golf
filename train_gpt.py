...
def main() -> None:
    ...
    if args.embed_weight_decay > 0.0 and not args.tie_embeddings:
        raise ValueError("EMBED_WEIGHT_DECAY>0 requires TIE_EMBEDDINGS=1 so the run targets the shared tok_emb matrix")
    ...
    token_optimizer_cls = torch.optim.AdamW if args.embed_weight_decay > 0.0 else torch.optim.Adam
    optimizer_tok = token_optimizer_cls(
    ...
    log0(
        "optimizer_weight_decay_scope:"
        f"{'tied_tok_emb_only' if args.embed_weight_decay > 0.0 else 'none'} "
        f"tok_optimizer:{token_optimizer_cls.__name__} "
        "head_weight_decay:0.0 scalar_weight_decay:0.0 matrix_weight_decay:0.0"
    )
    log0(
