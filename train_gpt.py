...
def main() -> None:
    ...
    if args.scalar_lr_warmdown_steps > 0:
        log0(
            "scalar_lr_schedule:"
            f" warmdown_start_step:{args.scalar_lr_warmdown_start_step}"
            f" warmdown_steps:{args.scalar_lr_warmdown_steps}"
            f" warmdown_target:{args.scalar_lr_warmdown_target:.5f}"
            " scope:scalar_control_only"
            " optimizer_step_semantics:pre_update"
            " warmdown_start_step_semantics:first_affected_update"
            f" last_untouched_checkpoint_step:{args.scalar_lr_warmdown_start_step}"
            f" first_affected_checkpoint_step:{args.scalar_lr_warmdown_start_step + 1}"
    ...
    def scalar_lr_mult(step: int) -> float:
        ...
        if step < args.scalar_lr_warmdown_start_step:
            return 1.0
        if step >= args.scalar_lr_warmdown_start_step + args.scalar_lr_warmdown_steps:
            return args.scalar_lr_warmdown_target
        progress = (step - args.scalar_lr_warmdown_start_step + 1) / max(args.scalar_lr_warmdown_steps, 1)
    ...
    while True:
        ...
        if should_validate:
            ...
            log0(
                ...
                f"lr_scale:{last_lr_scale:.6f} scalar_lr_mult:{last_scalar_lr_mult:.6f} "
                f"scalar_lr:{last_scalar_lr:.8f}"
            )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        scalar_scale = scalar_lr_mult(step)
        zero_grad_all()
