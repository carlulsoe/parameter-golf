...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA2 override requires TIE_EMBEDDINGS=1 so optimizer_tok stays on the shared token/logit path")
    ...
    def log_optimizer_betas(stage: str) -> None:
        head_optimizer = optimizer_head if base_model.lm_head is not None else None
        log0(
            "optimizer_betas "
            f"stage:{stage} "
            f"tok:{format_optimizer_betas(optimizer_tok)} "
            f"head:{format_optimizer_betas(head_optimizer)} "
    ...
    log0(
        f"optimizer_tok_scope tensors:1 numel:{base_model.tok_emb.weight.numel()} "
    ...
    log0(
        f"token_beta2_audit completed_updates:{step} token_beta2:{optimizer_tok.param_groups[0]['betas'][1]:.5f} "
