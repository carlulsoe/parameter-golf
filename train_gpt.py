...
def main() -> None:
    ...
    tok_param_count = sum(p.numel() for group in optimizer_tok.param_groups for p in group["params"])
    ...
    log0(
        f"optimizer_tok_scope tensors:{sum(len(group['params']) for group in optimizer_tok.param_groups)} "
        f"numel:{tok_param_count} tie_embeddings:{args.tie_embeddings}"
    )
