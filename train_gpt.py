...
def main() -> None:
    ...
    if args.token_beta1 != args.beta1 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA1 requires TIE_EMBEDDINGS=1 so it stays scoped to the tied embedding/logit matrix")
    ...
    def log_optimizer_beta_state(stage: str) -> None:
    ...
    log0(
        ...
        f"beta1:{args.beta1:.5f} "
        f"token_beta1:{args.token_beta1:.5f} "
        f"beta2:{args.beta2:.5f} "
        f"adam_eps:{args.adam_eps:.8f}"
    )
    log_optimizer_beta_state("startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_optimizer_beta_state("post_restore_startup")
    ...
    log_optimizer_beta_state("final")
