...
def main() -> None:
    ...
    non_resid_mix_scalar_params = [p for p in scalar_params if id(p) not in resid_mix_param_ids]
    ...
    log0(
        "resid_mix_grad_scale_config: "
        f"scale:{args.resid_mix_grad_scale:.5f} "
        f"tensors:{len(resid_mix_params)} "
        f"numel:{sum(int(p.numel()) for p in resid_mix_params)} "
        f"other_scalar_tensors:{len(non_resid_mix_scalar_params)} "
        f"other_scalar_numel:{sum(int(p.numel()) for p in non_resid_mix_scalar_params)}"
    )
    if resid_mix_named_params:
        log0(
            "resid_mix_grad_scale_names:" + ",".join(f"blocks.{name}" if not name.startswith("blocks.") else name for name, _ in resid_mix_named_params)
        )
    log0(
