...
def build_scalar_family_named_params(base_model: nn.Module) -> dict[str, list[tuple[str, nn.Parameter]]]:
...
def main() -> None:
    ...
    scalar_family_counts = {
        family_name: (
            len(named_params),
            sum(param.numel() for _, param in named_params),
        )
        for family_name, named_params in scalar_family_named_params.items()
    }
    ...
    def log_scalar_optimizer_audit(stage: str, *, completed_updates: int | None = None) -> None:
        if not args.scalar_family_audit:
            return
        ...
        skip_in_scalar_group = any(param is base_model.skip_weights for param in scalar_group_params)
        ...
        log0(
            ...
            f"scalar_numel:{scalar_numel} skip_weights_in_scalar_group:{skip_in_scalar_group} "
            f"skip_weights_tensors:{scalar_family_counts['skip_weights'][0]} "
            f"skip_weights_numel:{scalar_family_counts['skip_weights'][1]} "
            f"scalar_lr:{optimizer_scalar.param_groups[0]['lr']:.8f} "
            f"skip_weights_lr:{optimizer_scalar.param_groups[0]['lr']:.8f}{completed_updates_text}"
        )
    ...
    if args.scalar_family_audit:
        log0(
            ...
            f"attn_scale_tensors:{scalar_family_counts['attn_scale'][0]} "
            f"attn_scale_numel:{scalar_family_counts['attn_scale'][1]} "
            f"mlp_scale_tensors:{scalar_family_counts['mlp_scale'][0]} "
            f"mlp_scale_numel:{scalar_family_counts['mlp_scale'][1]} "
            f"resid_mix_tensors:{scalar_family_counts['resid_mix'][0]} "
            f"resid_mix_numel:{scalar_family_counts['resid_mix'][1]} "
            f"q_gain_tensors:{scalar_family_counts['q_gain'][0]} "
            f"q_gain_numel:{scalar_family_counts['q_gain'][1]} "
            f"skip_weights_tensors:{scalar_family_counts['skip_weights'][0]} "
            f"skip_weights_numel:{scalar_family_counts['skip_weights'][1]} "
    ...
    while True:
        ...
        scalar_family_pre_step_snapshots = None
        scalar_family_pre_step_norms = None
        zero_grad_all()
        ...
        if args.scalar_family_audit and should_log_train:
            ...
            scalar_family_pre_step_norms = {
                family_name: tensor_group_l2_norm(snapshots, device=device)
                for family_name, snapshots in scalar_family_pre_step_snapshots.items()
            }
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
        ...
        scalar_family_grad_ratios = None
        if args.scalar_family_audit and should_log_train and scalar_family_pre_step_norms is not None:
            scalar_family_grad_ratios = {}
            for family_name, named_params in scalar_family_named_params.items():
                ...
                denom = scalar_family_pre_step_norms[family_name].clamp_min(1e-12)
                scalar_family_grad_ratios[family_name] = (grad_norm / denom).item()
        if args.grad_clip_norm > 0:
            torch.nn.utils.clip_grad_norm_(base_model.parameters(), args.grad_clip_norm)
        for opt in optimizers:
            opt.step()
        scalar_family_update_ratios = None
        if (
            args.scalar_family_audit
            and should_log_train
            and scalar_family_pre_step_snapshots is not None
            and scalar_family_pre_step_norms is not None
        ):
            scalar_family_update_ratios = {}
            for family_name, named_params in scalar_family_named_params.items():
                update_sq = torch.zeros((), device=device, dtype=torch.float32)
                for (_, param), snapshot in zip(named_params, scalar_family_pre_step_snapshots[family_name], strict=True):
                    update_sq += torch.sum((param.detach().float() - snapshot).square())
                denom = scalar_family_pre_step_norms[family_name].clamp_min(1e-12)
                scalar_family_update_ratios[family_name] = (update_sq.sqrt() / denom).item()
        zero_grad_all()
        ...
        if should_log_train:
            ...
            if (
                args.scalar_family_audit
                and scalar_family_grad_ratios is not None
                and scalar_family_update_ratios is not None
            ):
                log0(
                    "scalar_family_audit_step "
                    f"step:{step} "
                    f"attn_scale_grad_to_param:{scalar_family_grad_ratios['attn_scale']:.8f} "
                    f"attn_scale_update_to_param:{scalar_family_update_ratios['attn_scale']:.8f} "
                    f"mlp_scale_grad_to_param:{scalar_family_grad_ratios['mlp_scale']:.8f} "
                    f"mlp_scale_update_to_param:{scalar_family_update_ratios['mlp_scale']:.8f} "
                    f"resid_mix_grad_to_param:{scalar_family_grad_ratios['resid_mix']:.8f} "
                    f"resid_mix_update_to_param:{scalar_family_update_ratios['resid_mix']:.8f} "
                    f"q_gain_grad_to_param:{scalar_family_grad_ratios['q_gain']:.8f} "
                    f"q_gain_update_to_param:{scalar_family_update_ratios['q_gain']:.8f} "
                    f"skip_weights_grad_to_param:{scalar_family_grad_ratios['skip_weights']:.8f} "
                    f"skip_weights_update_to_param:{scalar_family_update_ratios['skip_weights']:.8f}"
                )
    ...
    if args.scalar_family_audit and scalar_family_init_snapshots is not None:
        final_fields = []
        for family_name, named_params in scalar_family_named_params.items():
            drift_sq = torch.zeros((), dtype=torch.float32)
            init_sq = torch.zeros((), dtype=torch.float32)
            for (_, param), init_snapshot in zip(named_params, scalar_family_init_snapshots[family_name], strict=True):
        ...
        log0(f"scalar_family_audit_final completed_updates:{step} {' '.join(final_fields)}")
