...
def main() -> None:
    ...
    def optimizer_betas_str(opt: torch.optim.Optimizer | None) -> str:
        if opt is None:
            return "inactive"
    ...
    def log_optimizer_betas(stage: str) -> None:
        log0(
            ...
            f"tok:{optimizer_betas_str(optimizer_tok)} "
            f"scalar:{optimizer_betas_str(optimizer_scalar)} "
            f"head:{optimizer_betas_str(optimizer_head)} "
            "muon:none"
        )
    ...
    log0(
        f"optimizer_scalar_group stage:startup tensors:{len(scalar_params)} "
        f"numel:{sum(int(p.numel()) for p in scalar_params)} scalar_beta2:{args.scalar_beta2:.5f}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log0(
            f"optimizer_scalar_group stage:post_restore_startup tensors:{len(scalar_params)} "
            f"numel:{sum(int(p.numel()) for p in scalar_params)} scalar_beta2:{optimizer_scalar.param_groups[0]['betas'][1]:.5f}"
        )
    ...
    log0(
        f"optimizer_scalar_group stage:final tensors:{len(scalar_params)} "
        f"numel:{sum(int(p.numel()) for p in scalar_params)} "
        f"scalar_beta2:{optimizer_scalar.param_groups[0]['betas'][1]:.5f} completed_updates:{step}"
    )
