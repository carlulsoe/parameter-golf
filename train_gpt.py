...
class Hyperparameters:
    ...
    muon_nesterov = bool(int(os.environ.get("MUON_NESTEROV", "1")))
...
def main() -> None:
    ...
    log0(
        f"optimizer_muon: tensors:{len(matrix_params)} "
        f"numel:{sum(p.numel() for p in matrix_params)} "
        f"nesterov:{args.muon_nesterov} "
    ...
    log0(
        f"optimizer_muon_final: tensors:{len(matrix_params)} "
        f"numel:{sum(p.numel() for p in matrix_params)} "
        f"nesterov:{args.muon_nesterov} "
        f"last_momentum:{muon_momentum:.5f}"
    )
