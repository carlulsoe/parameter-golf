...
def main() -> None:
    ...
    if args.token_beta1 != args.beta1 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA1 overrides require TIE_EMBEDDINGS=1 so the knob stays on the shared token/logit path")
    ...
    log0(
        f"optimizer_betas: tok=({args.token_beta1:.5f},{args.beta2:.5f}) "
        f"scalar=({args.beta1:.5f},{args.beta2:.5f}) "
        f"head={'inactive' if base_model.lm_head is None else f'({args.beta1:.5f},{args.beta2:.5f})'} "
        f"token_scope:{'tied_only' if args.tie_embeddings else 'input_only'}"
    )
    log0(
    ...
    log0(
        f"optimizer_tok_final: tie_embeddings:{args.tie_embeddings} "
