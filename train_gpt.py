...
def main() -> None:
    ...
    log0(
        ...
        f"start_step:{args.scalar_lr_warmdown_start_step} warmdown_steps:{args.scalar_lr_warmdown_steps} "
        f"target:{args.scalar_lr_warmdown_target:.5f} end_step:{scalar_lr_warmdown_end_step} "
        f"step_semantics:applied_step_one_based scope:optimizer_scalar"
    )
    log0(
    ...
    def scalar_lr_mult(applied_step: int) -> float:
        if not scalar_lr_warmdown_enabled or applied_step < args.scalar_lr_warmdown_start_step:
            return 1.0
        if applied_step >= args.scalar_lr_warmdown_start_step + args.scalar_lr_warmdown_steps:
            return args.scalar_lr_warmdown_target
        progress = (applied_step - args.scalar_lr_warmdown_start_step + 1) / max(args.scalar_lr_warmdown_steps, 1)
    ...
    scalar_lr_first_activation_step: int | None = None
    ...
    while True:
        ...
        applied_step = step + 1
        scalar_scale = scalar_lr_mult(applied_step)
        if scalar_lr_first_activation_step is None and scalar_scale < 1.0:
            scalar_lr_first_activation_step = applied_step
    ...
    log0(
        ...
        f"configured_start_step:{args.scalar_lr_warmdown_start_step} "
        f"configured_warmdown_steps:{args.scalar_lr_warmdown_steps} "
        f"configured_target:{args.scalar_lr_warmdown_target:.5f} "
        f"first_activation_step:{scalar_lr_first_activation_step if scalar_lr_first_activation_step is not None else 0} "
