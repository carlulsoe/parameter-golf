...
def late_linear_warmdown_multiplier(step: int, start_step: int, warmdown_steps: int, target: float) -> float:
    if warmdown_steps <= 0 or step < start_step:
        return 1.0
    if warmdown_steps == 1:
        return target
    frac = min((step - start_step) / max(warmdown_steps - 1, 1), 1.0)
    return 1.0 + frac * (target - 1.0)
...
def main() -> None:
    ...
    log0(
        f"scalar_lr_schedule start_step:{args.scalar_lr_warmdown_start_step} "
        f"warmdown_steps:{args.scalar_lr_warmdown_steps} target:{args.scalar_lr_warmdown_target:.5f}"
    )
    log0(
    ...
    while True:
        ...
        scalar_lr_mult = late_linear_warmdown_multiplier(
            step,
            args.scalar_lr_warmdown_start_step,
            args.scalar_lr_warmdown_steps,
            args.scalar_lr_warmdown_target,
        )
        ...
        for group in optimizer_scalar.param_groups:
            group["lr"] = group["base_lr"] * scale * scalar_lr_mult
            last_scalar_lr = group["lr"]
        ...
        if should_log_train:
            log0(
                ...
                f"scalar_lr_mult:{scalar_lr_mult:.5f} scalar_lr:{last_scalar_lr:.8f}"
            )
    ...
    log0(
        f"scalar_lr_audit completed_updates:{step} start_step:{args.scalar_lr_warmdown_start_step} "
        f"warmdown_steps:{args.scalar_lr_warmdown_steps} target:{args.scalar_lr_warmdown_target:.5f} "
        f"last_scalar_lr_mult:{last_scalar_lr_mult:.5f} last_scalar_lr:{last_scalar_lr:.8f}"
    )
