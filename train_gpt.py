...
def main() -> None:
    ...
    if not (0.0 <= args.attn_scale_beta2 < 1.0):
        raise ValueError(f"ATTN_SCALE_BETA2 must be in [0, 1), got {args.attn_scale_beta2}")
    ...
    attn_scale_named_params: list[tuple[str, Tensor]] = []
    for name, p in block_named_params:
        if not (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)):
            continue
        if attn_scale_split_active and name.endswith("attn_scale"):
            attn_scale_named_params.append((name, p))
        else:
            scalar_named_params.append((name, p))
    ...
    attn_scale_numel = sum(int(p.numel()) for p in attn_scale_params)
    log0(
    ...
    log0(
        ...
        f" attn_scale_tensors:{len(attn_scale_params)} attn_scale_numel:{attn_scale_numel}"
        f" scalar_lr:{args.scalar_lr:.8f} attn_scale_lr:{args.scalar_lr:.8f}"
    )
    if attn_scale_split_active:
        log0("optimizer_attn_scale_names:" + ",".join(name for name, _ in attn_scale_named_params))
    ...
    def log_optimizer_betas(stage: str) -> None:
        ...
        head_betas = "inactive" if head_group is None else f"({head_group['betas'][0]:.5f},{head_group['betas'][1]:.5f})"
        attn_scale_group = optimizer_attn_scale.param_groups[0] if optimizer_attn_scale is not None else None
        attn_scale_betas = (
            "inactive"
            if attn_scale_group is None
            else f"({attn_scale_group['betas'][0]:.5f},{attn_scale_group['betas'][1]:.5f})"
        )
        log0(
