...
def main() -> None:
    ...
    def matrix_lr_tail_mul(step: int) -> float:
        if args.matrix_lr_warmdown_steps <= 0:
            return 1.0
        if step < args.matrix_lr_warmdown_start_step:
            return 1.0
        tail_progress = min((step - args.matrix_lr_warmdown_start_step) / max(args.matrix_lr_warmdown_steps, 1), 1.0)
        return 1.0 + (args.matrix_lr_warmdown_target - 1.0) * tail_progress
    ...
    while True:
        ...
        if should_validate:
            ...
            current_matrix_lr_mult = matrix_lr_tail_mul(step)
            ...
            log0(
                ...
                f"matrix_lr_mult:{current_matrix_lr_mult:.5f} matrix_lr:{args.matrix_lr * current_scale * current_matrix_lr_mult:.6f}"
            )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        matrix_lr_mult = matrix_lr_tail_mul(step)
        zero_grad_all()
        ...
        if should_log_train:
            log0(
                ...
                f"matrix_lr_mult:{matrix_lr_mult:.5f} matrix_lr:{args.matrix_lr * scale * matrix_lr_mult:.6f}"
            )
