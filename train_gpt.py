...
def main() -> None:
    ...
    def log_optimizer_betas(stage: str) -> None:
        ...
        if base_model.lm_head is not None:
            ...
            head_summary = f"head:({head_betas[0]:.5f},{head_betas[1]:.5f})"
        else:
            head_summary = "head:inactive"
        log0(
    ...
    def log_optimizer_scalar_group(stage: str) -> None:
        live_scalar_params = optimizer_scalar.param_groups[0]["params"]
        live_scalar_numel = sum(int(p.numel()) for p in live_scalar_params)
        live_scalar_tensor_count = len(live_scalar_params)
        live_scalar_match = all(id(p) in scalar_param_ids for p in live_scalar_params) and (
            live_scalar_tensor_count == len(scalar_params)
        )
        log0(
            f"optimizer_scalar_group stage:{stage} "
            f"tensors:{live_scalar_tensor_count} numel:{live_scalar_numel} "
            f"beta1:{optimizer_scalar.param_groups[0]['betas'][0]:.5f} "
            f"beta2:{optimizer_scalar.param_groups[0]['betas'][1]:.5f} "
            f"exact_match:{live_scalar_match}"
        )

    log_optimizer_betas("startup")
    log_optimizer_scalar_group("startup")
    ...
    if args.warmup_steps > 0:
        ...
        log_optimizer_scalar_group("post_restore_startup")
    ...
    log_optimizer_scalar_group("final")
