...
def main() -> None:
    ...
    def current_matrix_lr_scale() -> float:
        matrix_group = optimizer_muon.param_groups[0]
        matrix_base_lr = float(matrix_group["base_lr"])
        if matrix_base_lr <= 0.0:
            return 0.0
        return float(matrix_group["lr"]) / matrix_base_lr
    ...
    def log_optimizer_lr_groups(stage: str) -> None:
        ...
        log0(
            "optimizer_lr_groups: "
            f"stage:{stage} "
            f"lr_scale:{current_matrix_lr_scale():.8f} "
    ...
    last_applied_lr_scale = current_matrix_lr_scale()
