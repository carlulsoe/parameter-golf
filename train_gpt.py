...
def main() -> None:
    ...
    if args.eval_seq_len <= 0:
        raise ValueError(f"EVAL_SEQ_LEN must be positive, got {args.eval_seq_len}")
    if not math.isfinite(args.q_gain_lr_mult) or args.q_gain_lr_mult <= 0:
    ...
    q_gain_params = [p for name, p in block_named_params if "q_gain" in name]
    scalar_params = [
        p
        for name, p in block_named_params
        if (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)) and "q_gain" not in name
    ]
    ...
    log0(
        f"q_gain_group_config tensors:{len(q_gain_params)} numel:{q_gain_numel} "
        f"lr_mult:{args.q_gain_lr_mult:.8f} base_lr:{args.scalar_lr * args.q_gain_lr_mult:.8f} "
        f"other_scalar_tensors:{len(scalar_params)} other_scalar_numel:{scalar_numel} "
        f"adam_invariance_note:separate_lr_changes_real_step_size"
    )
