...
def main() -> None:
    ...
    if args.token_beta1 != args.beta1 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA1 overrides require TIE_EMBEDDINGS=1 so the scope stays on the shared token/logit path")
    ...
    tok_param_names = ["tok_emb.weight"]

    def log_optimizer_adam_audit(stage: str) -> None:
        ...
        if base_model.lm_head is not None:
            ...
            head_betas = f"({head_beta1:.5f},{head_beta2:.5f})"
            head_eps = f"{optimizer_head.param_groups[0]['eps']:.8f}"
        else:
            head_betas = "inactive"
            head_eps = "0.00000000"
        tok_scope = "tied_only" if args.tie_embeddings else "embeddings_only"
        tok_numel = sum(p.numel() for p in optimizer_tok.param_groups[0]["params"])
        log0(
            f"optimizer_adam_config stage:{stage} default_beta1:{args.beta1:.5f} "
            f"token_beta1:{args.token_beta1:.5f} beta2:{args.beta2:.5f} "
            f"token_eps:{optimizer_tok.param_groups[0]['eps']:.8f} "
            f"scalar_eps:{optimizer_scalar.param_groups[0]['eps']:.8f} head_eps:{head_eps}"
        )
        log0(
            f"optimizer_adam_betas stage:{stage} tok:({tok_beta1:.5f},{tok_beta2:.5f}) "
            f"scalar:({scalar_beta1:.5f},{scalar_beta2:.5f}) head:{head_betas}"
        )
        log0(
            f"optimizer_tok_scope stage:{stage} tie_embeddings:{args.tie_embeddings} "
            f"scope:{tok_scope} tensors:{len(tok_param_names)} numel:{tok_numel} "
            f"names:{','.join(tok_param_names)}"
        )

    log_optimizer_adam_audit("startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_optimizer_adam_audit("post_restore_startup")
    ...
    log_optimizer_adam_audit("final")
