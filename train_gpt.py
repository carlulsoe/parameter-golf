...
def main() -> None:
    ...
    scalar_param_ids = {id(p) for _, p in scalar_named_params}
    matrix_params = [p for _, p in block_named_params if id(p) not in scalar_param_ids]
    scalar_non_control_params = [
        p for name, p in scalar_named_params if not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
    ]
    ...
    if scalar_non_control_params:
        optimizer_scalar_groups.append(
            {"params": scalar_non_control_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr, "group_name": "scalar"}
        )
    if control_params:
        optimizer_scalar_groups.append(
            {
    ...
    log0(
        f"optimizer_scalar_groups: scalar_tensors:{len(scalar_non_control_params)} "
        f"scalar_numel:{sum(int(p.numel()) for p in scalar_non_control_params)} "
