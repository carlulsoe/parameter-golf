...
def main() -> None:
    ...
    if not math.isfinite(args.muon_momentum_warmup_start):
        raise ValueError(
            f"MUON_MOMENTUM_WARMUP_START must be finite, got {args.muon_momentum_warmup_start}"
        )
    ...
    def get_muon_momentum(applied_step: int) -> float:
    ...
    log0(
        ...
        f" step_semantics:zero_based_applied_step"
        ...
        f" step0:{get_muon_momentum(0):.5f}"
        f" step200:{get_muon_momentum(200):.5f}"
        f" warmup_end_step:{max(args.muon_momentum_warmup_steps, 1) - 1}"
    )
    ...
    last_applied_muon_momentum = float("nan")
    ...
    log0(
        ...
        f" last_applied_step:{max(step - 1, 0)}"
