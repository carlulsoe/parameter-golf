...
def main() -> None:
    ...
    def embed_lr_warmdown_mul(update_step: int) -> float:
        if args.embed_lr_warmdown_steps <= 0 or update_step < args.embed_lr_warmdown_start_step:
            return 1.0
        # warmdown_start_step is the first affected pre-update step; checkpoint step N
        # therefore reports the value that was applied on pre-update step N-1.
        progress = min(
            (update_step - args.embed_lr_warmdown_start_step + 1) / max(args.embed_lr_warmdown_steps, 1),
            1.0,
        )
