...
def main() -> None:
    ...
    if args.embed_beta1 != args.beta1 and not args.tie_embeddings:
        raise ValueError("EMBED_BETA1 requires TIE_EMBEDDINGS=1 when it differs from BETA1")
    ...
    log0(
        f"optimizer_betas:beta1:{args.beta1} embed_beta1:{args.embed_beta1} "
        f"beta2:{args.beta2} adam_eps:{args.adam_eps}"
    )
    log0(
        f"embed_beta1_scope:{'tied_tok_emb_only' if args.tie_embeddings else 'tok_emb_only'} "
