...
def main() -> None:
    ...
    log0(
        "optimizer_betas:"
        f"beta1:{args.beta1} beta2:{args.beta2} adam_eps:{args.adam_eps} "
        f"embed_lr_warmdown_target:{args.embed_lr_warmdown_target:.5f}"
    )
    log0(
        "embed_lr_schedule:"
        ...
        " scope:optimizer_tok_only"
        " optimizer_step_semantics:pre_update"
        " warmdown_start_step_semantics:first_affected_update"
        f" last_untouched_checkpoint_step:{args.embed_lr_warmdown_start_step}"
        f" first_affected_checkpoint_step:{args.embed_lr_warmdown_start_step + 1}"
    ...
    def measured_step_warmdown_mult(step: int, start_step: int, warmdown_steps: int, target: float) -> float:
        if warmdown_steps <= 0 or step < start_step:
            return 1.0
        frac = min((step - start_step + 1) / max(warmdown_steps, 1), 1.0)
        return 1.0 + (target - 1.0) * frac
    ...
    while True:
        ...
        embed_lr_mult = measured_step_warmdown_mult(
            step,
            args.embed_lr_warmdown_start_step,
            args.embed_lr_warmdown_steps,
            args.embed_lr_warmdown_target,
        )
        zero_grad_all()
        ...
        for opt in optimizers:
            for group in opt.param_groups:
                extra_mult = embed_lr_mult if opt is optimizer_tok else 1.0
                group["lr"] = group["base_lr"] * scale * extra_mult
        last_applied_embed_lr_mult = embed_lr_mult
