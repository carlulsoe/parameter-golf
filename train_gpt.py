...
def main() -> None:
    ...
    def adam_group_betas(optimizer: torch.optim.Optimizer | None) -> str:
        if optimizer is None or not optimizer.param_groups:
            return "inactive"
        beta1, beta2 = optimizer.param_groups[0]["betas"]
        return f"({beta1:.5f},{beta2:.5f})"

    def log_optimizer_betas(stage: str, optimizer_head: torch.optim.Optimizer | None = None) -> None:
        log0(
            f"optimizer_betas stage:{stage} "
            f"optimizer_tok:{adam_group_betas(optimizer_tok)} "
            f"optimizer_scalar:{adam_group_betas(optimizer_scalar)} "
            f"optimizer_head:{adam_group_betas(optimizer_head)} "
    ...
    validate_adam_beta("SCALAR_BETA1", args.scalar_beta1)
    ...
    log0(
        f"optimizer_beta_config beta1:{args.beta1:.5f} scalar_beta1:{args.scalar_beta1:.5f} "
        f"beta2:{args.beta2:.5f} adam_eps:{args.adam_eps:.8f}"
    )
    log0(
        f"optimizer_group_scope scalar_tensors:{len(scalar_params)} "
        f"scalar_numel:{sum(p.numel() for p in scalar_params)}"
    )
    log_optimizer_betas(stage="startup", optimizer_head=optimizer_head if base_model.lm_head is not None else None)
    log0(
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    log_optimizer_betas(stage="post_restore_startup", optimizer_head=optimizer_head if base_model.lm_head is not None else None)
    ...
    log_optimizer_betas(stage="final", optimizer_head=optimizer_head if base_model.lm_head is not None else None)
