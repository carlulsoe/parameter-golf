...
def main() -> None:
    ...
    log0(
        f"optimizer_beta1s:adam_beta1:{args.beta1} scalar_beta1:{args.scalar_beta1} "
        f"adam_beta2:{args.beta2} muon_momentum:{args.muon_momentum}"
    )
    log0(
        f"optimizer_beta1_scope:optimizer_tok:{args.beta1} "
        f"optimizer_head:{args.beta1 if base_model.lm_head is not None else 'tied_to_tok_emb'} "
        f"optimizer_scalar:{args.scalar_beta1} optimizer_muon_momentum:{args.muon_momentum}"
    )
    log0(
