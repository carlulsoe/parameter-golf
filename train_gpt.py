...
def compute_muon_momentum(applied_step: int, args: Hyperparameters) -> tuple[float, float]:
    if args.muon_momentum_warmup_steps > 0:
        frac = min(applied_step / args.muon_momentum_warmup_steps, 1.0)
    else:
        frac = 1.0
...
def main() -> None:
    ...
    if args.eval_seq_len <= 0:
        raise ValueError(f"EVAL_SEQ_LEN must be positive, got {args.eval_seq_len}")
    if not math.isfinite(args.muon_momentum_warmup_start) or not 0.0 <= args.muon_momentum_warmup_start < 1.0:
        raise ValueError(
            f"MUON_MOMENTUM_WARMUP_START must be finite and in [0, 1), got {args.muon_momentum_warmup_start}"
        )
    if not math.isfinite(args.muon_momentum) or not 0.0 <= args.muon_momentum < 1.0:
        raise ValueError(f"MUON_MOMENTUM must be finite and in [0, 1), got {args.muon_momentum}")
    if args.muon_momentum_warmup_steps < 0:
        raise ValueError(
    ...
    def log_muon_momentum_state(stage: str) -> None:
        group_momenta = [float(group["momentum"]) for group in optimizer_muon.param_groups]
        if not group_momenta:
            log0(f"muon_momentum_state: stage:{stage} group_count:0")
            return
        log0(
            f"muon_momentum_state: stage:{stage} group_count:{len(group_momenta)} "
            f"momentum_min:{min(group_momenta):.5f} momentum_max:{max(group_momenta):.5f}"
        )
    ...
    warmup_target_step = args.muon_momentum_warmup_steps if args.muon_momentum_warmup_steps > 0 else 0
    log0(
        ...
        f"step_semantics:applied_step_zero_based target_reached_step:{warmup_target_step}"
    )
    log0(
    ...
    log_muon_momentum_state("startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_muon_momentum_state("post_restore_startup")
    ...
    last_applied_muon_momentum, last_applied_muon_frac = compute_muon_momentum(0, args)
    ...
    while True:
        ...
        muon_momentum, muon_frac = compute_muon_momentum(step, args)
        for group in optimizer_muon.param_groups:
            group["momentum"] = muon_momentum
        last_applied_muon_momentum = muon_momentum
        last_applied_muon_frac = muon_frac
        ...
        if should_log_train:
            log0(
                ...
                f"muon_momentum:{last_applied_muon_momentum:.5f} muon_momentum_frac:{last_applied_muon_frac:.5f}"
            )
    ...
    log_muon_momentum_state("final")
    log0(
        f"muon_momentum_audit: last_applied_step:{step - 1 if step > 0 else -1} "
        f"last_applied_momentum:{last_applied_muon_momentum:.5f} "
        f"last_applied_frac:{last_applied_muon_frac:.5f}"
    )
