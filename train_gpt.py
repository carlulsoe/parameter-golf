...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA2 != BETA2 requires TIE_EMBEDDINGS=1 so the override stays on the shared token/logit matrix")
    ...
    def log_optimizer_tok_audit(stage: str) -> None:
        ...
        tok_names = "tok_emb.weight"
        tok_scope = "tied_shared_tok_emb_logit_group" if args.tie_embeddings else "untied_tok_emb_only"
        log0(
            f"optimizer_tok_audit stage:{stage} optimizer:Adam names:{tok_names} scope:{tok_scope} "
            f"beta1:{float(tok_group['betas'][0]):.5f} beta2:{float(tok_group['betas'][1]):.5f} "
