...
def main() -> None:
    ...
    if args.eval_seq_len <= 0:
        raise ValueError(f"EVAL_SEQ_LEN must be positive, got {args.eval_seq_len}")
    for name, value in (("BETA1", args.beta1), ("BETA2", args.beta2), ("EMBED_BETA2", args.embed_beta2)):
        if not (0.0 <= value < 1.0):
            raise ValueError(f"{name} must be in [0, 1), got {value}")
    if args.embed_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError(
            "EMBED_BETA2 != BETA2 requires TIE_EMBEDDINGS=1 so the run cannot silently test an untied-head path"
        )
    ...
    log0(
        f"optimizer_betas:beta1:{args.beta1} adam_beta2:{args.beta2} embed_beta2:{args.embed_beta2} "
        f"adam_eps:{args.adam_eps}"
    )
    log0(
        "optimizer_beta2_scope:"
        f"{'tied_tok_emb_only' if args.tie_embeddings and args.embed_beta2 != args.beta2 else 'baseline_all_groups'} "
