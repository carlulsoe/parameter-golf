...
def main() -> None:
    ...
    def log_optimizer_adam_betas(stage: str) -> None:
        ...
        if base_model.lm_head is not None:
            ...
            fields.append(f"head:({head_beta1:.5f},{head_beta2:.5f})")
        else:
            fields.append("head:none")
        log0("optimizer_adam_betas " + " ".join(fields))

    def log_optimizer_tok_scope(stage: str) -> None:
        tok_names = [name for name, param in base_model.named_parameters() if param is base_model.tok_emb.weight]
        log0(
            "optimizer_tok_scope "
            f"stage:{stage} "
            f"tie_embeddings:{args.tie_embeddings} "
            f"scope:{'tied_only' if args.tie_embeddings else 'input_only'} "
            f"tensors:{len(optimizer_tok.param_groups[0]['params'])} "
            f"numel:{sum(int(p.numel()) for p in optimizer_tok.param_groups[0]['params'])} "
