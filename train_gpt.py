...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA2 requires TIE_EMBEDDINGS=1 so the override stays on the shared embedding/logit path")
    ...
    def log_optimizer_beta_audit(stage: str) -> None:
        ...
        if base_model.lm_head is not None:
            head_group = next(
                (group for group in optimizers[1].param_groups if any(p is base_model.lm_head.weight for p in group["params"])),
                None,
            )
            if head_group is not None:
                head_betas = f"({head_group['betas'][0]:.5f},{head_group['betas'][1]:.5f})"
        tok_tensor_count = sum(len(group["params"]) for group in optimizer_tok.param_groups)
        tok_numel = sum(int(p.numel()) for group in optimizer_tok.param_groups for p in group["params"])
        log0(
        ...
        log0(
            f"optimizer_tok_scope stage:{stage} tensors:{tok_tensor_count} numel:{tok_numel} "
            f"tie_embeddings:{args.tie_embeddings} scope:{'tied_only' if args.tie_embeddings else 'input_only'}"
        )
