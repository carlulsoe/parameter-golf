...
def main() -> None:
    ...
    for beta_name, beta_value in (("BETA1", args.beta1), ("BETA2", args.beta2), ("CONTROL_BETA2", args.control_beta2)):
        if not math.isfinite(beta_value) or not (0.0 <= beta_value < 1.0):
            raise ValueError(f"{beta_name} must be finite and in [0, 1), got {beta_value}")
    ...
    named_param_lookup = {id(param): f"blocks.{name}" for name, param in block_named_params}
    named_param_lookup[id(base_model.skip_weights)] = "skip_weights"
    matrix_params = [
        p
    ...
    control_beta2_active = args.control_beta2 != args.beta2
    if control_beta2_active:
        scalar_param_groups = []
            ...
            scalar_param_groups.append(
                {
                    ...
                    "group_name": "other_scalar",
                }
            )
        if control_scalar_params:
            scalar_param_groups.append(
                {
                    "params": control_scalar_params,
                    "lr": args.scalar_lr,
                    "base_lr": args.scalar_lr,
                    "group_name": "control",
        ...
        optimizer_scalar = torch.optim.Adam(
            [{"params": other_scalar_params + control_scalar_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            fused=True,
        )
    ...
    log0(
        ...
        f"control_beta2:{args.control_beta2:.5f} adam_eps:{args.adam_eps:.8f}"
    )
    log0(
    ...
    def log_scalar_optimizer_groups(stage: str) -> None:
        if not control_beta2_active:
            return
        for idx, group in enumerate(optimizer_scalar.param_groups):
            group_name = str(group.get("group_name", f"group_{idx}"))
            tensor_names = [named_param_lookup.get(id(param), "<unknown>") for param in group["params"]]
            tensor_count = len(group["params"])
            numel = sum(int(param.numel()) for param in group["params"])
            contains_skip_weights = any(name == "skip_weights" for name in tensor_names)
            beta1, beta2 = group["betas"]
            log0(
                "optimizer_scalar_groups: "
                f"stage:{stage} group:{group_name} tensors:{tensor_count} numel:{numel} "
                f"beta1:{beta1:.5f} beta2:{beta2:.5f} contains_skip_weights:{contains_skip_weights}"
            )
