...
def main() -> None:
    ...
    if args.matrix_lr_warmdown_steps > 0 and args.matrix_lr_warmdown_target < 1.0:
        log0(
            "matrix_lr_schedule: "
            f"enabled:True start_step:{args.matrix_lr_warmdown_start_step} "
            f"warmdown_steps:{args.matrix_lr_warmdown_steps} "
            f"target:{args.matrix_lr_warmdown_target:.5f} "
            f"end_step:{args.matrix_lr_warmdown_start_step + args.matrix_lr_warmdown_steps - 1} "
            "scope:optimizer_muon step_semantics:optimizer_step_zero_based"
        )
    ...
    def matrix_lr_mul(step: int) -> float:
        if args.matrix_lr_warmdown_steps <= 0 or args.matrix_lr_warmdown_target >= 1.0:
            return 1.0
        ...
        progress = min(
            (step - args.matrix_lr_warmdown_start_step) / max(args.matrix_lr_warmdown_steps - 1, 1),
            1.0,
        )
    ...
    while True:
        ...
        for group in optimizer_muon.param_groups:
            group["lr"] = group["base_lr"] * scale * matrix_scale
        ...
        if should_log_train:
            ...
            if args.matrix_lr_warmdown_steps > 0 and args.matrix_lr_warmdown_target < 1.0:
                train_log += (
                    f" matrix_lr_mult:{matrix_scale:.5f} "
