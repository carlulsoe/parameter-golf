...
def main() -> None:
    ...
    log0(
        ...
        f"target:{args.scalar_lr_warmdown_target:.5f} scope:optimizer_scalar"
    )
    ...
    def scalar_lr_mul(step: int) -> float:
        if not scalar_lr_warmdown_enabled:
            return 1.0
        ...
        frac = progress / (args.scalar_lr_warmdown_steps - 1)
        return 1.0 + frac * (args.scalar_lr_warmdown_target - 1.0)
    ...
    log0(
        f"scalar_lr_audit: completed_updates:{step} "
        f"warmdown_enabled:{scalar_lr_warmdown_enabled} "
        f"start_step:{args.scalar_lr_warmdown_start_step} "
        f"warmdown_steps:{args.scalar_lr_warmdown_steps} "
