...
def main() -> None:
    ...
    def log_optimizer_group_audit(stage: str) -> None:
        scalar_names = [
            param_to_name.get(id(param), "<unknown>")
        ...
        skip_names = []
        skip_numel = 0
        skip_beta2 = args.beta2
        if optimizer_skip_weights is not None:
            skip_names = [
                param_to_name.get(id(param), "<unknown>")
                for group in optimizer_skip_weights.param_groups
                for param in group["params"]
            ]
            skip_numel = sum(
                int(param.numel())
            ...
            skip_beta2 = optimizer_skip_weights.param_groups[0]["betas"][1]
        ...
        log0(
            ...
            f"skip_weights_tensors:{len(skip_names)} skip_weights_numel:{skip_numel}"
        )
        log0(
            ...
            f"skip_weights:({args.beta1:.5f},{skip_beta2:.5f}) "
            f"head:{'inactive' if base_model.lm_head is None else f'({optimizer_head.param_groups[0]['betas'][0]:.5f},{optimizer_head.param_groups[0]['betas'][1]:.5f})'}"
        )
        if skip_names:
            log0(f"optimizer_skip_weights_names stage:{stage} names:{','.join(skip_names)}")
    ...
    log0(
        ...
        f"skip_weights_beta2:{args.skip_weights_beta2:.5f} adam_eps:{args.adam_eps:.8f}"
    )
