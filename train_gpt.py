...
def main() -> None:
    ...
    if ema_audit_enabled:
        log0(
            ...
            "pinned_auto_keep_from_raw:True"
        )
    else:
    ...
    log0(
        "training_stop_audit: "
        f"completed_steps:{step} stop_after_step:{stop_after_step if stop_after_step is not None else -1} "
        f"train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms / max(step, 1):.2f}ms "
        f"ema_updates:{ema_update_count} ema_tracked_tensors:{ema_tracked_tensors} "
    ...
    quant_obj, quant_stats = quantize_state_dict_int8(raw_state_dict)
    ...
    if ema_audit_enabled and ema_state_dict is not None:
        ...
        torch.cuda.synchronize()
        ema_quant_obj, ema_quant_stats = quantize_state_dict_int8(
            ema_state_dict_cpu,
            selected_auto_keep_name_override=raw_selected_name or None,
        )
        base_model.load_state_dict(dequantize_state_dict_int8(ema_quant_obj), strict=True)
        torch.cuda.synchronize()
        ...
        log0(
            f"EMA audit int8_pinned_exact val_loss:{ema_q_val_loss:.8f} val_bpb:{ema_q_val_bpb:.8f} "
    ...
    log0(
        "final_int8_zlib_roundtrip_gap: "
        f"val_loss_delta:{q_val_loss - last_val_loss:.8f} "
        f"val_bpb_delta:{q_val_bpb - last_val_bpb:.8f}"
    )
