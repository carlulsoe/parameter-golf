...
def main() -> None:
    ...
    if args.eval_seq_len <= 0:
        raise ValueError(f"EVAL_SEQ_LEN must be positive, got {args.eval_seq_len}")
    if not 0.0 <= args.embed_beta2 < 1.0:
        raise ValueError(f"EMBED_BETA2 must be in [0, 1), got {args.embed_beta2}")
    if not args.tie_embeddings and not math.isclose(args.embed_beta2, args.beta2, rel_tol=0.0, abs_tol=0.0):
        raise ValueError(
            "EMBED_BETA2 must equal BETA2 when TIE_EMBEDDINGS=0 because optimizer_tok then covers input "
            f"embeddings only; got EMBED_BETA2={args.embed_beta2} and BETA2={args.beta2}"
        )
    ...
    log0(
        ...
        f"adam_beta2:{args.beta2} embed_beta2:{args.embed_beta2}"
    )
    log0(
        "optimizer_beta2_scope: "
        f"optimizer_tok:{args.embed_beta2} "
        f"optimizer_head:{args.beta2 if base_model.lm_head is not None else 'n/a'} "
        f"optimizer_scalar:{args.beta2} "
        f"muon_unchanged:{args.muon_momentum:.5f} "
