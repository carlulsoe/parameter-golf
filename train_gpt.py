...
def main() -> None:
    ...
    scalar_param_tensors = len(control_scalar_params) + len(other_scalar_params)
    scalar_param_numel = sum(int(p.numel()) for p in control_scalar_params + other_scalar_params)
    split_control_lr = (
        args.control_lr_scale != 1.0 and len(control_scalar_params) > 0 and len(other_scalar_params) > 0
    )
    if split_control_lr:
            ...
            [
                {
                    ...
                    "lr": args.scalar_lr * args.control_lr_scale,
                    "base_lr": args.scalar_lr * args.control_lr_scale,
                }
            ],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
        ...
        optimizers: list[torch.optim.Optimizer] = [
            optimizer_tok,
            optimizer_muon,
            optimizer_scalar_other,
            optimizer_scalar_control,
        ]
    else:
        optimizer_scalar = torch.optim.Adam(
            [
                {
                    "params": other_scalar_params + control_scalar_params,
                    "lr": args.scalar_lr,
    ...
    log0(
        "scalar_group_audit: "
        f"control_param_tensors:{len(control_scalar_params)} "
        f"control_param_numel:{sum(int(p.numel()) for p in control_scalar_params)} "
        f"other_scalar_param_tensors:{len(other_scalar_params)} "
        f"other_scalar_param_numel:{sum(int(p.numel()) for p in other_scalar_params)} "
    ...
    log0(
        ...
        + (
            f"split_control_lr control_lr:{args.scalar_lr * args.control_lr_scale:.6f} "
            f"other_scalar_lr:{args.scalar_lr:.6f}"
            if split_control_lr
            else f"combined_scalar_lr scalar_lr:{args.scalar_lr:.6f}"
        )
    )
    log0(
