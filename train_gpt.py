...
class DistributedTokenLoader:
    ...
    def shard_order_preview(self, limit: int = 4) -> str:
        return ",".join(file.name for file in self.stream.files[: max(limit, 0)])
...
def main() -> None:
    ...
    log0(
        ...
        f"seed:{args.seed} first_shards:{train_loader.shard_order_preview()}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        log0(
            f"train_loader_post_warmup_order: shuffle_train_shards:{args.shuffle_train_shards} "
            f"seed:{args.seed} first_shards:{train_loader.shard_order_preview()}"
        )
