...
class TokenStream:
    ...
    def load_state_dict(self, state: dict[str, int]) -> None:
        ...
        self.wrap_count = int(state.get("wrap_count", 0))
        self.total_tokens_taken = int(state.get("total_tokens_taken", 0))
...
def main() -> None:
    ...
    warmup_per_rank_span_tokens = warmup_local_tokens + 1
    warmup_replay_tokens_per_rank = args.warmup_steps * grad_accum_steps * warmup_per_rank_span_tokens
    log0(
        "train_loader_warmup_policy: "
        f"reset_after_warmup:{args.reset_train_loader_after_warmup} "
        f"warmup_micro_steps:{args.warmup_steps * grad_accum_steps} "
        f"per_rank_span_tokens:{warmup_per_rank_span_tokens} "
    ...
    if args.warmup_steps > 0:
        ...
        if args.reset_train_loader_after_warmup:
            ...
            measured_loader_state = warmup_loader_state
        train_loader.load_state_dict(measured_loader_state)
        log0(
            "train_loader_after_warmup: "
            f"reset_after_warmup:{args.reset_train_loader_after_warmup} "
            f"warmup_file_idx:{warmup_loader_state['file_idx']} "
            f"warmup_pos:{warmup_loader_state['pos']} "
            f"warmup_wrap_count:{warmup_loader_state['wrap_count']} "
            f"warmup_total_tokens_taken:{warmup_loader_state['total_tokens_taken']} "
            f"measured_file_idx:{measured_loader_state['file_idx']} "
            f"measured_pos:{measured_loader_state['pos']} "
            f"measured_wrap_count:{measured_loader_state['wrap_count']} "
