...
def main() -> None:
    ...
    def param_group_l2_norm(named_params: list[tuple[str, Tensor]]) -> float:
        total = torch.zeros((), device=device, dtype=torch.float64)
        for _, param in named_params:
            total += param.detach().double().square().sum()
        return total.sqrt().item()

    scalar_param_l2_before = param_group_l2_norm(scalar_named_params)
    ...
    log0(
        f"optimizer_scalar_group:optimizer:{scalar_optimizer_cls.__name__} "
        f"tensors:{scalar_tensor_count} numel:{scalar_numel} "
        f"weight_decay:{args.scalar_weight_decay:.8f} "
        f"param_l2_norm_before:{scalar_param_l2_before:.8f}"
    )
    log0(
    ...
    while True:
        ...
        if args.scalar_weight_decay > 0:
            scalar_cumulative_lr_weight_decay += optimizer_scalar.param_groups[0]["lr"] * args.scalar_weight_decay
        for opt in optimizers:
            opt.step()
        zero_grad_all()
    ...
    log0(
        f"optimizer_scalar_decay_audit:tensors:{scalar_tensor_count} numel:{scalar_numel} "
        f"weight_decay:{args.scalar_weight_decay:.8f} "
        f"cumulative_lr_weight_decay:{scalar_cumulative_lr_weight_decay:.8f} "
        f"param_l2_norm_after:{param_group_l2_norm(scalar_named_params):.8f}"
    )
