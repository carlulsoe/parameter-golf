...
def main() -> None:
    ...
    if not 0.0 <= args.scalar_beta2 < 1.0:
        raise ValueError(f"SCALAR_BETA2 must satisfy 0 <= SCALAR_BETA2 < 1, got {args.scalar_beta2}")
    ...
    def log_optimizer_betas(stage: str) -> None:
        ...
        if optimizer_head is not None:
            ...
            head_beta_msg = f"({head_beta1:.5f},{head_beta2:.5f})"
        else:
            head_beta_msg = "inactive"
        scalar_group_params = optimizer_scalar.param_groups[0]["params"]
        scalar_numel = sum(int(p.numel()) for p in scalar_group_params)
        log0(
            f"optimizer_betas stage:{stage} "
            f"tok:({tok_beta1:.5f},{tok_beta2:.5f}) "
            f"scalar:({scalar_beta1:.5f},{scalar_beta2:.5f}) "
            f"head:{head_beta_msg}"
        )
        log0(
            f"optimizer_scalar_group stage:{stage} "
            f"tensors:{len(scalar_group_params)} numel:{scalar_numel}"
        )
