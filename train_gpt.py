...
class DistributedTokenLoader:
    ...
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device):
        ...
        self.last_global_tokens = 0
        self.last_grad_accum_steps = 0
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        self.last_grad_accum_steps = grad_accum_steps
        ...
        snapshot.update(
            {
                ...
                "last_local_tokens": self.last_local_tokens,
                "last_global_tokens": self.last_global_tokens,
                "last_grad_accum_steps": self.last_grad_accum_steps,
            }
        )
        return snapshot
    ...
    if args.train_start_shard_audit:
        ...
        if warmup_loader_snapshot is not None:
            ...
            for shard_offset in (1, 4, 8, 16, 32):
                ...
                offset_replay_tokens = unique_prefix_overlap(
                    start=int(shard_start_offsets[start_file_idx]),
                    length=measured_stream_tokens,
                    prefix_len=warmup_stream_tokens,
                    total_tokens=dataset_total_tokens,
                )
                offset_unique_union_tokens = min(
                    dataset_total_tokens,
                    min(warmup_stream_tokens, dataset_total_tokens)
                    + min(measured_stream_tokens, dataset_total_tokens)
                    - offset_replay_tokens,
                )
                log0(
                    "train_start_shard_counterfactual: "
                    f"offset:{shard_offset} "
                    f"start_file_idx:{start_file_idx} "
                    f"replay_tokens:{offset_replay_tokens} "
                    f"avoided_replay_tokens:{baseline_replay_tokens - offset_replay_tokens} "
                    f"unique_union_tokens:{offset_unique_union_tokens} "
