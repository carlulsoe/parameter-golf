...
def main() -> None:
    ...
    skip_weights_beta2_active = not math.isclose(args.skip_weights_beta2, args.beta2, rel_tol=0.0, abs_tol=1e-12)
    if base_model.skip_weights.numel() > 0 and not skip_weights_beta2_active:
        scalar_params.append(base_model.skip_weights)
    skip_weight_tensors = 1 if base_model.skip_weights.numel() > 0 else 0
    skip_weight_numel = int(base_model.skip_weights.numel())
    ...
    optimizer_skip_weights: torch.optim.Optimizer | None = None
    if base_model.skip_weights.numel() > 0 and skip_weights_beta2_active:
    ...
    log0(
        ...
        f"skip_weights_beta2:{args.skip_weights_beta2:.5f} adam_eps:{args.adam_eps:.8g}"
    )
    log0(
    ...
    log0(
        f"optimizer_scalar_groups: scalar_tensors:{len(scalar_params)} scalar_numel:{sum(p.numel() for p in scalar_params)} "
        f"skip_weights_split_active:{skip_weights_beta2_active} "
        f"skip_weight_tensors:{skip_weight_tensors} skip_weight_numel:{skip_weight_numel}"
    )
    log0(
        f"optimizer_skip_weights: active:{skip_weights_beta2_active} "
        f"beta2:{args.skip_weights_beta2:.5f} "
        f"lr:{args.scalar_lr:.8f} names:{'skip_weights' if skip_weight_tensors else 'none'}"
    )
    log0(
    ...
    final_skip_weights_lr = args.scalar_lr * scale if step > 0 else args.scalar_lr
    log0(
        f"optimizer_skip_weights_final: active:{skip_weights_beta2_active} "
        f"beta2:{args.skip_weights_beta2:.5f} "
        f"lr:{final_skip_weights_lr:.8f} completed_updates:{step}"
    )
