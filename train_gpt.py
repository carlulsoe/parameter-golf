...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA2 may differ from BETA2 only when TIE_EMBEDDINGS=1")
    ...
    def log_optimizer_tok_audit(stage: str) -> None:
        scope = "tied_shared_tok_emb_logit_group" if args.tie_embeddings else "untied_tok_emb_only"
        names: list[str] = []
        for group in optimizer_tok.param_groups:
            names.extend(param_name_by_id.get(id(param), "<unknown>") for param in group["params"])
        log0(
            f"optimizer_tok audit: stage:{stage} optimizer:Adam "
            f"tie_embeddings:{args.tie_embeddings} scope:{scope} "
            f"tensors:{sum(len(group['params']) for group in optimizer_tok.param_groups)} "
            f"names:{','.join(names) if names else 'none'} "
            f"beta2:{optimizer_tok.param_groups[0]['betas'][1]:.5f} "
            f"base_lr:{optimizer_tok.param_groups[0]['base_lr']:.8f} "
