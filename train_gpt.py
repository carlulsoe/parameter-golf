...
def main() -> None:
    ...
    def log_optimizer_tok_audit(stage: str) -> None:
        ...
        group_names = [named_params_by_id.get(id(param), "<unknown>") for param in group["params"]]
        lr = float(group["lr"])
        base_lr = float(group.get("base_lr", lr))
        beta1, beta2 = group["betas"]
        log0(
            "optimizer_tok audit: "
            f"stage:{stage} optimizer:{type(optimizer_tok).__name__} "
            f"scope:{'tied_shared_tok_emb_logit_group' if args.tie_embeddings else 'untied_tok_emb_only'} "
            f"tie_embeddings:{args.tie_embeddings} "
            f"tensors:{len(group['params'])} names:{','.join(group_names)} "
            f"beta1:{beta1:.5f} beta2:{beta2:.5f} "
            f"configured_token_lr:{token_lr:.8f} base_lr:{base_lr:.8f} lr:{lr:.8f}"
        )
