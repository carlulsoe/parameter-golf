...
for percentile in INT8_CLIP_SWEEP_AUDIT_PERCENTILES:
    if not 0.0 < percentile < 100.0:
        raise ValueError("INT8_CLIP_SWEEP_AUDIT_PERCENTILES entries must be strictly between 0 and 100")
if INT8_CLIP_SWEEP_AUDIT_MIN_TENSOR_GAIN < 0.0:
    raise ValueError("INT8_CLIP_SWEEP_AUDIT_MIN_TENSOR_GAIN must be non-negative")
if INT8_CLIP_SWEEP_AUDIT_MIN_IMPROVED_TENSORS < 0:
    raise ValueError("INT8_CLIP_SWEEP_AUDIT_MIN_IMPROVED_TENSORS must be non-negative")
if INT8_CLIP_SWEEP_AUDIT_MIN_MEAN_GAIN < 0.0:
    raise ValueError("INT8_CLIP_SWEEP_AUDIT_MIN_MEAN_GAIN must be non-negative")
if INT8_CLIP_SWEEP_AUDIT_MIN_MEDIAN_GAIN < 0.0:
    raise ValueError("INT8_CLIP_SWEEP_AUDIT_MIN_MEDIAN_GAIN must be non-negative")
if INT8_CLIP_SWEEP_AUDIT_REQUIRE_MATCHED_TENSORS < 0:
    raise ValueError("INT8_CLIP_SWEEP_AUDIT_REQUIRE_MATCHED_TENSORS must be non-negative")
...
def audit_clip_sweep_family(
    ...
    candidate_percentiles = tuple(
        percentile
        for percentile in INT8_CLIP_SWEEP_AUDIT_PERCENTILES
        if abs(percentile - INT8_CLIP_PERCENTILE) > 1e-12
    )
    ...
    if not candidate_percentiles or not matched_tensors:
        return {
    ...
    for percentile in candidate_percentiles:
        ...
        for item in matched_tensors:
            ...
            candidate_q, candidate_s = quantize_float_tensor(
                ...
                min_clip_value=float(item["min_clip_value"]),
                clip_q=percentile / 100.0,
            )
            candidate_error = normalized_mae(t, dequantize_quantized_tensor(candidate_q, candidate_s, dtype=t.dtype))
            gain = float(item["baseline_error"]) - candidate_error
            per_tensor_results.append(
                {
                    "name": str(item["name"]),
                    "baseline_error": float(item["baseline_error"]),
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    stats["clip_sweep_audit_candidate_count"] = int(clip_sweep_audit["candidate_count"]) if clip_sweep_audit is not None else 0
    ...
    stats["clip_sweep_audit_tensor_summary"] = str(clip_sweep_audit["tensor_summary"]) if clip_sweep_audit is not None else ""
