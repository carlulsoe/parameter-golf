...
def main() -> None:
    ...
    args = Hyperparameters()
    for name, value in (("BETA1", args.beta1), ("TOKEN_BETA1", args.token_beta1), ("BETA2", args.beta2)):
    ...
    log0(
        f"optimizer_adam_config beta1:{args.beta1:.5f} "
        f"token_beta1:{args.token_beta1:.5f} beta2:{args.beta2:.5f} adam_eps:{args.adam_eps:.0e}"
    )
    log0(
    ...
    def log_optimizer_beta_scope(stage: str) -> None:
        ...
        token_scope = "tied_shared_matrix" if args.tie_embeddings else "untied_input_only"
        log0(
            f"optimizer_tok_scope stage:{stage} tie_embeddings:{args.tie_embeddings} "
            f"scope:{token_scope} tensors:1 numel:{base_model.tok_emb.weight.numel()} "
            f"names:tok_emb.weight"
        )

    log_optimizer_beta_scope("startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_optimizer_beta_scope("post_restore_startup")
    ...
    log_optimizer_beta_scope("final")
