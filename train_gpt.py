...
class Hyperparameters:
    ...
    token_beta2 = float(os.environ.get("TOKEN_BETA2", str(beta2)))
...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA2 != BETA2 requires TIE_EMBEDDINGS=1 so optimizer_tok stays on the shared embedding/logit matrix")
    ...
    optimizer_head_beta1 = "inactive" if base_model.lm_head is None else f"{args.beta1:.5f}"
    optimizer_head_beta2 = "inactive" if base_model.lm_head is None else f"{args.beta2:.5f}"
    log0(
        "optimizer_beta_scope "
        f"optimizer_tok_beta1:{args.beta1:.5f} optimizer_tok_beta2:{args.token_beta2:.5f} "
        f"optimizer_scalar_beta1:{args.beta1:.5f} optimizer_scalar_beta2:{args.beta2:.5f} "
        f"optimizer_head_beta1:{optimizer_head_beta1} optimizer_head_beta2:{optimizer_head_beta2} "
