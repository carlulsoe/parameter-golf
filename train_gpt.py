...
def main() -> None:
    ...
    if args.token_beta2 != args.beta2 and not args.tie_embeddings:
        raise ValueError(
            "TOKEN_BETA2 can differ from BETA2 only when TIE_EMBEDDINGS=1, "
            f"got TIE_EMBEDDINGS={int(args.tie_embeddings)} TOKEN_BETA2={args.token_beta2} BETA2={args.beta2}"
        )
    ...
    log0(
        ...
        f"optimizer_tok:{args.token_beta2:.5f} "
        f"optimizer_head:{head_beta2_scope} "
        f"optimizer_scalar:{args.beta2:.5f} "
        f"muon_beta2:none "
