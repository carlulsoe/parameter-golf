...
def main() -> None:
    ...
    scalar_family_names = sorted(
        {
            name.split(".", 2)[-1] if name.startswith("blocks.") else name
            for name, _ in scalar_named_params
        }
    )
    ...
    def log_optimizer_scalar_scope(stage: str) -> None:
        log0(
            f"optimizer_scalar_scope stage:{stage} "
            f"tensors:{len(scalar_named_params)} "
            f"numel:{sum(int(p.numel()) for _, p in scalar_named_params)} "
            f"families:{','.join(scalar_family_names)}"
        )
    ...
    log0(
        ...
        f"scalar_beta2:{args.scalar_beta2:.5f} adam_eps:{args.adam_eps:.8g}"
    )
    log0(
