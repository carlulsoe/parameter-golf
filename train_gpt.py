...
def main() -> None:
    ...
    if args.scalar_lr_warmdown_target <= 0:
        raise ValueError(f"SCALAR_LR_WARMDOWN_TARGET must be positive, got {args.scalar_lr_warmdown_target}")
    ...
    log0(
        f"scalar_lr_schedule:warmdown_start_step:{args.scalar_lr_warmdown_start_step} "
    ...
    def step_warmdown_mul(step: int, start_step: int, warmdown_steps: int, target: float) -> float:
        if warmdown_steps <= 0:
            return 1.0
        if step < start_step:
            return 1.0
        progress = min((step - start_step + 1) / max(warmdown_steps, 1), 1.0)
    ...
    while True:
        ...
        if should_validate:
            ...
            log0(
                ...
                f"scalar_lr_mult:{last_scalar_lr_mult:.5f} scalar_lr:{optimizer_scalar.param_groups[0]['lr']:.6f}"
            )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        scalar_lr_mult = step_warmdown_mul(
            step,
