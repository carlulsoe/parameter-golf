...
def log_optimizer_scalar_group(
    log0,
    stage: str,
    optimizer_scalar: torch.optim.Optimizer,
    skip_weights: Tensor,
) -> None:
    scalar_params = [param for group in optimizer_scalar.param_groups for param in group["params"]]
    scalar_numel = sum(int(param.numel()) for param in scalar_params)
    scalar_lr = float(optimizer_scalar.param_groups[0]["lr"]) if optimizer_scalar.param_groups else 0.0
    skip_weights_in_scalar_group = any(param is skip_weights for param in scalar_params)
    skip_weights_lr = 0.0
    for group in optimizer_scalar.param_groups:
        if any(param is skip_weights for param in group["params"]):
            skip_weights_lr = float(group["lr"])
            break
    log0(
        ...
        f"skip_weights_in_scalar_group:{skip_weights_in_scalar_group} "
        f"scalar_lr:{scalar_lr:.8f} "
...
def main() -> None:
    ...
    if args.scalar_family_audit:
        log_optimizer_scalar_group(log0, "startup", optimizer_scalar, base_model.skip_weights)
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        if args.scalar_family_audit:
            log_optimizer_scalar_group(log0, "post_restore_startup", optimizer_scalar, base_model.skip_weights)
    ...
    if args.scalar_family_audit:
        log_optimizer_scalar_group(log0, "final", optimizer_scalar, base_model.skip_weights)
