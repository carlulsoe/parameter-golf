...
def main() -> None:
    ...
    if args.token_eps != args.adam_eps and not args.tie_embeddings:
        raise ValueError("TOKEN_EPS override requires TIE_EMBEDDINGS=1")
    ...
    token_param_numel = int(base_model.tok_emb.weight.numel())

    def log_optimizer_beta_eps_audit(stage: str) -> None:
        tok_betas = optimizer_tok.param_groups[0]["betas"]
        tok_eps = float(optimizer_tok.param_groups[0]["eps"])
        scalar_betas = optimizer_scalar.param_groups[0]["betas"]
        scalar_eps = float(optimizer_scalar.param_groups[0]["eps"])
        if base_model.lm_head is not None:
            head_betas = optimizer_head.param_groups[0]["betas"]
            head_eps = float(optimizer_head.param_groups[0]["eps"])
            head_summary = f"head:({head_betas[0]:.5f},{head_betas[1]:.5f}) head_eps:{head_eps:.8f}"
        else:
            head_summary = "head:none head_eps:none"
        log0(
            "optimizer_beta_eps_audit: "
            f"stage:{stage} "
            f"tok:({tok_betas[0]:.5f},{tok_betas[1]:.5f}) tok_eps:{tok_eps:.8f} "
            f"scalar:({scalar_betas[0]:.5f},{scalar_betas[1]:.5f}) scalar_eps:{scalar_eps:.8f} "
            f"{head_summary}"
        )
    ...
    def log_optimizer_tok_scope(stage: str) -> None:
        log0(
            "optimizer_tok_scope: "
            f"stage:{stage} tensors:1 numel:{token_param_numel} "
    ...
    log0(
        "optimizer_eps_config: "
        f"beta1:{args.beta1:.5f} beta2:{args.beta2:.5f} "
        f"adam_eps:{args.adam_eps:.8f} token_eps:{args.token_eps:.8f}"
    )
    log0(
    ...
    log_optimizer_beta_eps_audit("startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_optimizer_beta_eps_audit("post_restore_startup")
    ...
    log_optimizer_beta_eps_audit("final")
