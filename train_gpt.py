...
def main() -> None:
    ...
    if args.token_beta1 != args.beta1 and not args.tie_embeddings:
        raise ValueError("TOKEN_BETA1 override requires TIE_EMBEDDINGS=1")
    ...
    def log_optimizer_adam_betas(stage: str) -> None:
        head_betas = format_betas(optimizer_head) if base_model.lm_head is not None else "none"
        log0(
            f"optimizer_adam_betas:stage:{stage} tok:{format_betas(optimizer_tok)} "
            f"scalar:{format_betas(optimizer_scalar)} head:{head_betas}"
        )
        log0(
            f"optimizer_tok_scope:stage:{stage} tie_embeddings:{args.tie_embeddings} "
            f"scope:{'tied_only' if args.tie_embeddings else 'input_only'} tensors:1 "
            f"numel:{base_model.tok_emb.weight.numel()} names:tok_emb.weight"
        )
    ...
    log0(
        f"optimizer_adam_config:default_beta1:{args.beta1:.5f} token_beta1:{args.token_beta1:.5f} "
        f"beta2:{args.beta2:.5f}"
    )
    log0(
    ...
    log_optimizer_adam_betas(stage="startup")
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
        log_optimizer_adam_betas(stage="post_restore_startup")
    ...
    log_optimizer_adam_betas(stage="final")
